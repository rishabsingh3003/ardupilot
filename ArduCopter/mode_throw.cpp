...
bool ModeThrow::throw_detected()
{
    ...
    bool throw_condition_confirmed = ((AP_HAL::millis() - free_fall_start_ms < 500) && ((inertial_nav.get_velocity_z_up_cms() - free_fall_start_velz) < -75.0f));

    
    // gcs().send_text(MAV_SEVERITY_CRITICAL, "speed %5.3f, fall %5.3f", (double)possible_throw_detected, (double)throw_condition_confirmed);
