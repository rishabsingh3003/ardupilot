...
void Mode::land_run_vertical_control(bool pause_descent)
{
    ...
    if (!pause_descent) {
        ...
        if (doing_precision_landing) {
            ...
            if (copter.rangefinder_alt_ok() && copter.rangefinder_state.alt_cm < 20.0f) {
                cmb_rate = 0.0f;
            } else if (target_error_cm > max_horiz_pos_error_cm && !is_zero(max_horiz_pos_error_cm)) {
                // doing precland but too far away from the obstacle
                // do not descend
                cmb_rate = 0.0f;
            } else if (copter.rangefinder_alt_ok() && copter.rangefinder_state.alt_cm > 15.0f && copter.rangefinder_state.alt_cm < 200.0f) {
