...
// Run precland statemachine. This function should be called from any mode that wants to land.
...
void Mode::run_precland()
{
    ...
    if (!copter.ap.land_repo_active) {
        ...
        Vector3f retry_pos;
        const AC_PrecLand_StateMachine::Status landing_action = copter.precland_statemachine.update(retry_pos);

        switch (landing_action) {
        ...
        case AC_PrecLand_StateMachine::Status::FAILSAFE: {
            ...
            AC_PrecLand_StateMachine::FailSafeAction fs_action = copter.precland_statemachine.get_failsafe_actions();
            switch (fs_action) {
