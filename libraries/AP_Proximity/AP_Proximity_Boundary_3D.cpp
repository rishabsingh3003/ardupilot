...
void AP_Proximity_Boundary_3D::apply_filter_cutoff_freq(float cutoff_freq)
{
    for (uint8_t layer=0; layer < PROXIMITY_NUM_LAYERS; layer++) {
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
...
void AP_Proximity_Boundary_3D::set_filtered_distance(const Face &face, float distance)
{
    ...
    if (!is_equal(_filtered_distance[face.layer][face.sector].get_cutoff_freq(), _filter_cutoff_freq)) {
        // cutoff freq has changed
        apply_filter_cutoff_freq(_filter_cutoff_freq);
    }
    ...
    for (uint8_t layer=0; layer < PROXIMITY_NUM_LAYERS; layer++) {
        for (uint8_t sector=0; sector < PROXIMITY_NUM_SECTORS; sector++) {
            if (_distance_valid[layer][sector]) {
                const uint32_t dt =  AP_HAL::millis() - _last_update_ms[layer][sector];
                if (dt > PROXIMITY_FACE_RESET_MS) {
