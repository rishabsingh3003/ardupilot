...
class AP_Proximity_LightWareSF45B_I2C : public AP_Proximity_Backend
{
...
private:
    ...
    bool configure_sensor(uint32_t configuration);
    bool get_configuration(uint32_t &configuration);
    bool get_reading(uint16_t &reading_mm, uint16_t &signal_strength, uint16_t &status);
