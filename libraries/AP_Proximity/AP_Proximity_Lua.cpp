...
// handle script distance messages. Returns false if distance is negative
bool AP_Proximity_Lua::handle_script_distance_msg(float dist_m, float yaw_deg, float pitch_deg, bool push_to_boundary)
{
    ...
    if (is_negative(dist_m)) {
        // message isn't healthy
        return false;
    }

    if (dist_m < distance_min() || dist_m > distance_max()) {
        // out of range or invalid so early return
        return true;
    }

    if (ignore_reading(pitch_deg, yaw_deg, dist_m, false)) {
        // obstacle is probably near ground or out of range, early return
        return true;
    }
