...
void AP_Proximity_LightWareSF40C::process_message()
{
    ...
    case MessageID::DISTANCE_OUTPUT: {
        ...
        float current_pitch;
        const bool database_ready = database_prepare_for_push(current_pos, current_heading,current_pitch);
        ...
        for (uint16_t i = 0; i < point_count; i++) {
            ...
            if ((i+1 >= point_count) || (combined_count >= PROXIMITY_SF40C_COMBINE_READINGS)) {
                if ((combined_dist_m < INT16_MAX) && database_ready) {
                    database_push(combined_angle_deg, combined_dist_m, _last_distance_received_ms, current_pos, current_heading, current_pitch);
                }
                combined_count = 0;
