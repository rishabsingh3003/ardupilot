...
protected:
    ...
    bool database_prepare_for_push(Vector3f &current_pos, float &current_heading, float &current_pitch);
    void database_push(float angle, float distance);
    void database_push(float angle, float distance, uint32_t timestamp_ms, const Vector3f &current_pos, float current_heading, float current_pitch);
