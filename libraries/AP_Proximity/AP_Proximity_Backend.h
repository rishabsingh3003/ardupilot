...
public:
    ...
    bool find_closest_point_to_boundary(const uint8_t sector, const uint8_t layer, Vector3f& vec_to_boundary) const { return boundary.find_closest_point_to_boundary(sector, layer, vec_to_boundary); }
    float find_closest_point_to_boundary_from_segment(const uint8_t sector, const uint8_t layer, const Vector3f& seg_start, const Vector3f& seg_end, Vector3f& closest_point) const { return boundary.find_closest_point_to_boundary_from_segment(sector, layer, seg_start, seg_end, closest_point); } 
    ...
    uint8_t get_object_count() const {return boundary.get_object_count(); };
    bool get_object_angle_and_distance(uint8_t object_number, float& angle_deg, float &distance) const { return boundary.get_object_angle_and_distance(object_number, angle_deg, distance); }
