...
#define PROXIMITY_LAND_DETECT_THRESHOLD 1.0f
...
public:
    ...
    uint8_t get_layers() const { return boundary.get_layers(); }
    mutable bool     _land_detected; // for logging
protected:
    ...
    bool check_obstacle_near_land(float yaw, float pitch, float distance) const;
    bool check_obstacle_near_land(float yaw, float distance) const { return check_obstacle_near_land(yaw, 0.0f, distance); };
    // Check if Obstacle defined by Vector3f is near land. The vector is assumed to be body frame FRD
    bool check_obstacle_near_land(const Vector3f &obstacle) const;
