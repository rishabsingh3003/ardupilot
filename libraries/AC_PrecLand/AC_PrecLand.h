...
public:
    ...
    // return the last known landing location in Earth Frame NEU.
    void get_last_detected_landing_location(Vector3f &landing_loc) const { landing_loc = _last_target_loc_rel_origin; }

    // Parameter getters
    PldRetryStrictness get_plnd_retry_strictness() const { return static_cast<PldRetryStrictness>(_strict.get());  }
...
private:
    ...
    Vector3f                    _last_target_loc_rel_origin;  // stores the last known location of the target horizontally, and the height of the vehicle where it detected this target in meters NEU
