...
const AP_Param::GroupInfo AC_PrecLand::var_info[] = {
    ...
    // @Param: RT_MAX
    // @DisplayName: PrecLand Maximum number of retires to a failed landing
    // @Description: PrecLand Maximum number of retires to a failed landing. Set zero to disable landing retry.
    // @Range: 0 10
    // @Increment: 1
    ...
    // @Description: Vehicle will continue landing vertically even if target is lost below this height. This needs a rangefinder to work. Set 0 to disable this.
    // @Range: 0 5
    // @Units: m
    AP_GROUPINFO("MIN_HEIGHT", 13, AC_PrecLand, _sensor_min_alt, 0.75),

    // @Param: MAX_HEIGHT
    // @DisplayName: PrecLand maximum height for retry
    // @Description: Vehicle will continue landing vertically till this height if target not found. Below this height is landing target is not found, landing retry/failsafe might be attempted. This needs a rangefinder to work. Set 0 to disable this.
    // @Range: 0 50
    // @Units: m
    AP_GROUPINFO("MAX_HEIGHT", 14, AC_PrecLand, _sensor_max_alt, 8),
...
void AC_PrecLand::init(uint16_t update_rate_hz)
{
    ...
    _current_status = PldState::TARGET_LOST;
...
void AC_PrecLand::check_target_status(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
    if (target_acquired()) {
        ...
        return;
    } else if (_current_status == PldState::TARGET_FOUND || _current_status == PldState::TARGET_RECENTLY_LOST) {
    ...
    // We definelty do not have the target in sight since we have reached here
    ...
    if (_current_status == PldState::TARGET_RECENTLY_LOST) {
        // check if its nearby/found recently, else the status will be demoted to "TARGET_LOST"
        Vector2f curr_pos;
        if (AP::ahrs().get_relative_position_NE_origin(curr_pos)) {
            const float dist_to_last_loc_xy = (curr_pos - Vector2f{_last_target_loc_rel_origin.x, _last_target_loc_rel_origin.y} * 0.01f).length();
            if ((dist_to_last_loc_xy > LANDING_TARGET_LOST_DIST_THRESH_M) || (AP_HAL::millis() - _last_precland_output_ms) > LANDING_TARGET_LOST_TIMEOUT_MS) {
                // the last known location of target is too far away, or it has been a long time since we haven't seen it
                // might as well consider it as "lost"
                _current_status = PldState::TARGET_LOST;
                return;
            }
        }
...
bool AC_PrecLand::check_if_sensor_in_range(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
    ...
    if (rangefinder_alt_m > _sensor_max_alt && !is_zero(_sensor_max_alt)) {
        ...
        return false;
    } else if (rangefinder_alt_m < _sensor_min_alt && !is_zero(_sensor_min_alt)) {
...
bool AC_PrecLand::construct_pos_meas_using_rangefinder(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
    ...
    if (retrieve_los_meas(target_vec_unit_body)) {
        ...
        if (target_vec_valid && alt_valid) {
            ...
            float z_origin;
            if (_ahrs.get_relative_position_D_origin(z_origin)) {
                _last_target_loc_rel_origin.z = z_origin * -100.0f;
            }
            return true;
        }
    }
...
void AC_PrecLand::run_output_prediction()
{
    ...
    _last_target_loc_rel_origin.x = last_target_loc_rel_origin_2d.x;
    _last_target_loc_rel_origin.y = last_target_loc_rel_origin_2d.y;
