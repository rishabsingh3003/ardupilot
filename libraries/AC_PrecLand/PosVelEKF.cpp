...
void PosVelEKF_AB::init(float pos, float posVar, float vel, float velVar)
{
    _state[0] = pos;
    _state[1] = vel;
    _state[2] = 0.0f;
...
void PosVelEKF_AB::predict(float dt, float dVel, float dVelNoise)
{
...
void PosVelEKF_AB::fusePos(float pos, float posVar)
{
    float newState[3];
    float newCov[6];
...
float PosVelEKF_AB::getPosNIS(float pos, float posVar)
{
...
// Initialize the covariance and state matrix
// This is called when the landing target is located for the first time or it was lost, then relocated
void PosVelEKF_refactor::init(float pos, float posVar, float vel, float velVar)
{
    _state[0] = pos;
    _state[1] = vel;
    _cov[0] = posVar;
...
// This functions runs the Prediction Step of the EKF
// This is called at 400 hz
void PosVelEKF_refactor::predict(float dt, float dVel, float dVelNoise)
{
    // Newly predicted state and covariance matrix at next time step
    float newState[2];
    float newCov[3];
...
// fuse the new sensor measurement into the EKF calculations
// This is called whenever we have a new measurement available
void PosVelEKF_refactor::fusePos(float pos, float posVar)
{
    float newState[2];
    float newCov[3];
...
// Returns normalized innovation squared
float PosVelEKF_refactor::getPosNIS(float pos, float posVar)
{
    // NIS = innovation_residual.Transpose * Innovation_Covariance.Inverse * innovation_residual
    const float innovation_residual = pos - _state[0];
    const float innovation_covariance = _cov[0] + posVar;
