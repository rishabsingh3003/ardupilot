...
// Parameters: Vector3f "retry_pos_m" is filled with the required location if we need to retry landing.
AC_PrecLand_StateMachine::Status AC_PrecLand_StateMachine::do_target_lost_actions(Vector3f &retry_pos_m)
{
    AC_PrecLand *_precland = AP::ac_precland();
    if (_precland == nullptr) {
        // should never happen
    ...
    case TargetLostAction::INIT: {
        ...
        RetryStrictness strictness =_precland->get_retry_strictness();
        switch (strictness) {
...
AC_PrecLand_StateMachine::Status AC_PrecLand_StateMachine::retry_landing(Vector3f &retry_pos_m)
{
    ...
    if (_retry_count > _precland->get_max_retry_allowed() || _precland->get_max_retry_allowed() == 0) {
    ...
    Vector3f go_to_pos;
    RetryAction retry_action = _precland->get_retry_behaviour();
