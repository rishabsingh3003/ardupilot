...
private:
    ...
    bool do_dfs(uint16_t start, Vector3f quad_nodes[4], Graph &grouping_graph, bool visited[]);
    void create_nodes_from_objects(const AP_OADatabase::OA_DbItem& item, Vector3f quad_nodes[4]);
    ...
    Graph *_grouping_graph;
    Vector3f unit_offsets[4];
