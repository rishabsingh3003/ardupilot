    ...
    for (uint8_t sector = 0; sector<num_sector; sector++) {
        for (uint8_t layer = 0; layer<num_layers; layer++) {
            ...
            if (is_negative(dist_to_boundary - margin_cm)) {
                ...
                const Vector3f margin_vector = vector_to_boundary.normalized() * breach_dist;
                const float xy_back_dist = norm(margin_vector.x, margin_vector.y);
                const float z_back_dist = margin_vector.z;
                calc_backup_velocity_3D(kP, accel_cmss, quad_1_back_vel, quad_2_back_vel, quad_3_back_vel, quad_4_back_vel, xy_back_dist, vector_to_boundary, kP_z, accel_cmss_z, z_back_dist, min_back_vel_z, max_back_vel_z, dt);
            }
