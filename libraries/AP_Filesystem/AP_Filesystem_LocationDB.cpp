...
/*
  packed format:
    ...
    per-entry is LocationDB DBItem
 */
...
AP_Filesystem_LocationDB::packet_t AP_Filesystem_LocationDB::construct_packet_from_DBItem(AP_LocationDB::DBItem item)
{   
    packet_t ret;
    ...
    float heading;
    if (item.get_heading(heading)) {
        ret.fields[field_index++] = heading;
    }
    
    float radius;
    if (item.get_radius(radius)) {
...
/*
  get one item
 */
bool AP_Filesystem_LocationDB::get_item(uint16_t idx, AP_LocationDB::DBItem &item) const
{
    auto *db = AP::locationdb();
    if (!db) {
        return false;
