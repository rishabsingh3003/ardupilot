...
private:
    ...
    // get one item
    bool get_item(uint16_t idx, AP_LocationDB::DBItem &item) const;
    packet_t construct_packet_from_DBItem(AP_LocationDB::DBItem item);
    bool skip_byte(AP_LocationDB::DBItem &item, uint32_t byte_num) const;
};
