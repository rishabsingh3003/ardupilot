...
{
    // direction vectors
    const Vector3f line1 = seg1_end - seg1_start;
    const Vector3f line2 = seg2_end - seg2_start;

    const Vector3f diff = seg1_start - seg2_start;
    ...
    const Vector3f closest_line_segment = diff + (line1*sc) - (line2*tc);
...
template Vector3<float> Vector3<float>::point_on_line_closest_to_other_point(const Vector3<float> &w1, const Vector3<float> &w2, const Vector3<float> &p);
